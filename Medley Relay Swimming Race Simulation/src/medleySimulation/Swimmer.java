...
public class Swimmer extends Thread {
	...
	// barrier for the first 10 swimmers
	private static final CyclicBarrier startingBarrier = new CyclicBarrier(10);

	// Bool flags for each lane/staring grid, ensures one swimmer per lane at a time
	private static final boolean[] startingGrid = new boolean[10];
	...
	private final SwimTeam swimTeam;
	...
	Swimmer( int ID, int t, PeopleLocation loc, FinishCounter f, int speed, SwimStroke s,SwimTeam swimTeam) {
		...
		// Initially Make all lanes/starting blocks free at the start
		for (int i = 0; i < startingGrid.length; i++) {
			startingGrid[i] = true; // true - as in: it is true that this block is unoccupied
		}
	}
	...
	public void run() {
		try {
			// Wait for this swimmer's turn based on swim stroke order
			synchronized (swimTeam) {
				while (swimStroke.getOrder() != swimTeam.getOrder()) {
					swimTeam.wait();  // Wait until it's this swimmer's turn
				}
			}
			...
			synchronized (swimTeam) {
				swimTeam.incrementOrder(); // get the next stroke to compare with
				swimTeam.notifyAll();  // Notify other swimmers waiting for their turn
			}
	
			// Go to the starting blocks
			goToStartingBlocks();
	
			// Wait for 10 swimmers (1 from each team) to reach the starting blocks
			startingBarrier.await();
	
			// Wait for the start of the lane to be free
			synchronized (startingGrid) {
				while (!startingGrid[team]) {  // while the block is not free
					startingGrid.wait();  // Wait until the previous swimmer in this lane finishes
				}
				
				startingGrid[team] = false; // Occupy the start block
			}
			...
			// If the swimmer is the last one (freestyle), mark the race as finished
			if (swimStroke.getOrder() == 4) {
				finish.finishRace(ID, team);
			} else {
				// When coming back to exit the pool release the start block for the next swimmer
				synchronized (startingGrid) {
					startingGrid[team] = true;  // Mark the lane as free
					startingGrid.notifyAll();  // Notify the next swimmer
				}
