...
public class StadiumView extends JPanel implements Runnable {
		...
		StadiumView(PeopleLocation[] people,  StadiumGrid grid, FinishCounter finishCounter) { //constructor
			this.finishC = finishCounter;
		...
		public void paintComponent(Graphics g) {
	       ...
	       */
		   ...
		   //draw the ovals representing people in middle of grid block
			...
			/*
			 * THis for drawing the Green line for the winner
			 */
			if (finishC.isRaceWon()) {
				int winningTeam = finishC.getWinningTeam() - 1;
				int laneStart = winningTeam * 5;  // Assuming each lane is 5 blocks wide
		
				// Color the entire lane red for the winning team
				g.setColor(Color.GREEN);
				for (int z = laneStart; z < laneStart + 5; z++) {
					g.fillRect((z + xBorder) * wIncr, yBorder * hIncr, wIncr, endPool * hIncr);
				}
				g.setColor(Color.WHITE);
				g.setFont(new Font("Helvetica", Font.BOLD, 20));
				g.drawString("1st", (xBorder-2 + (winningTeam * 5) + 2) * wIncr, hIncr * (endPool / 2 + yBorder));
			}
